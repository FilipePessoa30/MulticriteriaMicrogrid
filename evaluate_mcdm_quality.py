...
def empty_result(label: str) -> Dict[str, float]:
    return {
        ...
        "regret_topsis": float("nan"),
        "regret_vikor": float("nan"),
        "regret_copras": float("nan"),
        "regret_moora": float("nan"),
...
def evaluate_weights(
    ...
    regrets = {}
    for name, col in METHOD_SCORE_COLS.items():
        if col in result:
            regrets[f"regret_{name}"] = regret(result[col])
